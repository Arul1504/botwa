...
async function connectWhatsapp() {
  ...
  socket.ev.on("connection.update", async ({ connection }) => {
    if (connection === "open") {
      console.log("BOT WHATSAPP SUDAH SIAP✅ -- BY SKYVEE"); //memberitahu jika sudah connect
  ...
  socket.ev.on("messages.upsert", async ({ messages, type }) => {
    ...
    async function sendSticker(jid, url, quoted, options) {
    ...
    switch (command) {
      ...
      case ".hidetag":
        ...
        if (!chat.key.remoteJid.includes("@g.us")) {
          await socket.sendMessage(
            chat.key.remoteJid,
            { text: "*Command ini hanya bisa di gunakan di grub!!*" },
            { quoted: chat }
          );
          return;
        }
